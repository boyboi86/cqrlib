...
def make_randomt1_data(n_samples: int =10000, max_days: float = 5.):
    # generate a random dataset for a classification problem
    _today = dt.datetime.today()
    df0 = pd.date_range(periods=n_samples, freq=pd.tseries.offsets.BDay(), end=_today).to_numpy()
    rand_days = np.random.uniform(max_days/2, max_days, n_samples)
