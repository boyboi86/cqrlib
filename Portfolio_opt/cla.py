...
class CLA:
    ...
    def solve(self):
        ...
        #1) case a): Bound one free weight
            ...
            if len(f)>1:
                ...
                for i in f:
                    ...
                    if l>self._inf_none(l_in):l_in,i_in,bi_in=l,i,bi
                    j+=1
            ...
            if len(f)<self.mean.shape[0]:
                ...
                for i in b:
                    ...
                    l,bi=self.computeLambda(covarF_inv,covarFB,meanF,wB,meanF.shape[0]-1, \
                        self.w[-1][i])
                    if (self.l[-1] is None or l < self.l[-1]) and l > self._inf_none(l_out):l_out,i_out=l,i
                if (l_in is None or self._inf_none(l_in)<0) and (l_out is None or self._inf_none(l_out)<0):
                ...
                #4) decide lambda
                    if self._inf_none(l_in)> self._inf_none(l_out):
                        self.l.append(l_in)
                        f.remove(i_in)
        ...
        self.purgeNumErr(self.tol)
        self.purgeExcess()
        
        
    def _inf_none(self, num):
        return float("-inf") if num is None else num
    ...
    def purgeNumErr(self,tol):
        ...
        i=0
        while True:
            if i>=len(self.w):break # prevent uneven loop, as long as i is less than weight lenght we will break
            w=self.w[i]
